...

{
...

    {
        ...
        $mail->Subject = "Reset Password Verification Code: " . $RandomCode;
    ...
    function about_me($email, $about_me)

    {
        ...
        if ($result) {
            ...
            //$user["About_Me_Successfully_Updated"] = true;
            return $user;
        } else {
            //$user["About_Me_Successfully_Updated"] = false;
            return false;
        }
    }
...

    {
        ...
        $stmt = $this->conn->prepare("SELECT id FROM follow_list where username1 = ? ORDER BY id");
        ...
        for ($i = 0; $i < $rowcount; $i++) {
            ...
            $stmt = $this->conn->prepare("SELECT username1_user FROM follow_list where username1 = ? LIMIT 1 OFFSET ?");
    ...
    public function follow_or_unfollow($username, $other_user_username, $isFollowing_str)
    {

        if ($isFollowing_str == "f") {
            // User data is inserted in the database using query "INSERT INTO".
            // The query we used is known as Prepared Statements in MySQLi.
            // In our SQL, we insert a question mark (?) where we want to substitute in an integer, string, double or blob value.
            $stmt = $this->conn->prepare("INSERT INTO follow_list(username, other_user_username) VALUES(?, ?)");

            // This function binds the parameters to the SQL query and tells the database what the parameters are.
            // The â€œssssssâ€� argument lists the types of data that the parameters are.
            // The s character tells MySQL that the parameter is a string
            // By telling MySQL what type of data to expect, we minimize the risk of SQL injections.
            $stmt->bind_param("ss", $username, $other_user_username);
            // Checking successful storage using $result
            $result = $stmt->execute();
            $stmt->close();
            ...
            if ($result) {
                ...
                $stmt2 = $this->conn->prepare("UPDATE user_info SET numFollowing = numFollowing + 1 where username1 = ?");
                ...
                // Check for successful store
                if ($result2) {
                    $stmt3 = $this->conn->prepare("UPDATE user_info SET numFollowers = numFollowers + 1 where username1 = ?");
                    // This function binds the parameters to the SQL query and tells the database what the parameters are.
                    // The â€œssssssâ€� argument lists the types of data that the parameters are.
                    // The s character tells MySQL that the parameter is a string
                    // By telling MySQL what type of data to expect, we minimize the risk of SQL injections.
                    $stmt3->bind_param("s", $other_user_username);
                    // Checking successful storage using $result
                    ...
                    if ($result3) {
                        $user["error"]  = false;
                        $user["error_msg"]  = "User is successfully following other user!";
                        return $user;
                    } else {
                        $user["error"]  = true;
                        $user["error_msg"]  = "There is an error updating the number of people following this user.";
                        return $user;
                    }
                } else {
                    $user["error"]  = true;
                    $user["error_msg"]  = "There is an error updating the number of people you are following.";
                    return $user;
                }
            } else {
                $user["error"]  = true;
                $user["error_msg"]  = "There is an error following this user. Please try again later.";
                return $user;
            }
        } elseif ($isFollowing_str == "t") {
            // In our SQL, we insert a question mark (?) where we want to substitute in an integer, string, double or blob value.
            $stmt = $this->conn->prepare("DELETE FROM follow_list where username = ? and  other_user_username = ?");
            ...
            if ($result) {
                ...
                if ($result2) {
                    ...
                    if ($result3) {
                        $user["error"]  = false;
                        $user["error_msg"]  = "User is successfully unfollowing other user!";
                        return $user;
                    } else {
                        $user["error"]  = true;
                        $user["error_msg"]  = "There is an error updating the number of people following this user.";
                ...
                $user["error"]  = true;
                $user["error_msg"]  = "There is an error unfollowing this user. Please try again later.";
                return $user;
            }
        }
    }
    ...
    public function followcheck($username, $other_user_username)
    {
        // Entry is selected from table user_info according to username1 id provided.
        $stmt = $this->conn->prepare("SELECT username, other_user_username FROM follow_list WHERE username = ? AND other_user_username = ?");
        $stmt->bind_param("ss", $username, $other_user_username);
        ...
        if ($stmt->num_rows > 0) {
            ...
            // user is not following other user
            return false;
        }
        $stmt->close();
    }
    ...
    // public function follow_or_unfollow($email, $other_user_email, $isFollowing);
    // {
    //     // User data is inserted in the database using query "INSERT INTO".
    //     // The query we used is known as Prepared Statements in MySQLi.
    //     // In our SQL, we insert a question mark (?) where we want to substitute in an integer, string, double or blob value.
    ...
    public function userprofileinformation($other_user_email, $other_user_username1)

    {

        // $stmt = $this->conn->prepare("SELECT id FROM follow_list WHERE username1_user = ? ORDER BY id");
        // $stmt->bind_param("s", $username1_user);
        // $stmt->execute();
        // $stmt->store_result();
        // $NumberOfFollowers = $stmt->num_rows;
        // $stmt->close();
        ...
        // $stmt->execute();
        // $stmt->store_result();
        // $NumberOfFollowing = $stmt->num_rows;
        // $stmt->close();
        ...
        // $stmt->execute();
        // $stmt->store_result();
        // $NumberOfPosts = $stmt->num_rows;
        // $stmt->close();

        // $stmt = $this->conn->prepare("SELECT id FROM post_comments WHERE username_comments = ? ORDER BY id");
        // $stmt->bind_param("s", $username1_user);
        // $stmt->execute();
        // $stmt->store_result();
        // $NumberOfComments = $stmt->num_rows;
        // $stmt->close();

        // Entry is selected from table user_info according to username1 id provided.
        $stmt = $this->conn->prepare("SELECT id, first_name, last_name, username1, email, profile_pic_ref, banner_pic_ref, About_Me, username1_lowercase, dateandtime, current_location, numPosts, numComments, numLikes, numDislikes, numFollowers, numFollowing  FROM user_info WHERE username1 = ?");
        $stmt->bind_param("s", $other_user_username1);
...
<div>
...

    {
        ...
        if ($stmt->execute()) {
            ...
            if ($encrypted_password == $hash) {
                // User authentication details are correct
                // "HEEEEEEEEEEYYY YAAAAAAAAAAAALLLLLLLLLLLLLLL";
                return $user;
            } else {
                // If password matched then $user is returned otherwise NULL.
